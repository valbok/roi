...
type
  TfrmData = class(TfrmNav)
    ...
    miBy: TMenuItem;
    miBy2: TMenuItem;
    procedure setD(ds:tdatasource;mq:Tquery;Caption,miStr1,mistr2:string);
...
uses unDm;
{$R *.dfm}
procedure tfrmData.setD(ds:tdatasource;mQ:tquery;Caption,miStr1,mistr2:string);
begin
  grid.DataSource:=ds;
  masterQ:=mq;
...
begin
  ...
  begin
        if grid.DataSource=dsqVidi then
        begin
           setD(dsqtkani_by_vid,qtkani_by_vid,
        ...
        if (grid.DataSource=dsQtkani) or  (grid.DataSource=dsqtkani_by_vid )then
         begin
           setD(dsqzakazi_by_tkan,qzakazi_by_tkan,
        ...
        if (grid.DataSource=dsqizdeliya) then
         begin
           setD(dsqRask_by_izdelie,qrask_by_izdelie,
        ...
        if (grid.DataSource=dsQZakazchiki) then
         begin
           setD(dsqzakazi_by_zakazchik,qzakazi_by_zakazchik,
...
with dm do
        if (grid.DataSource=dsQizdeliya) then
         begin
           setD(dsqZakazi_by_izdelie,qZakazi_by_izdelie,
